...
# http://0.0.0.0:8080/api_clientes?user_hash=12345&action=put&id_cliente=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=0
    def put(self, nombre,apellidos,telefono):
        try:
            config.model.insert_clientes(nombre,apellidos,telefono)
            clientes_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(clientes_json)
        except Exception as e:
...
# http://0.0.0.0:8080/api_clientes?user_hash=12345&action=update&id_cliente=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
    def update(self, id_cliente, nombre,apellidos,telefono):
        try:
            config.model.edit_clientes(id_cliente,nombre,apellidos,telefono)
            clientes_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(clientes_json)
        except Exception as e:
        ...
        try:
            ...
            if user_hash == '12345':
                if action is None:
                    ...
                    return self.put(nombre,apellidos,telefono)
                elif action == 'delete':
                    return self.delete(id_cliente)
                elif action == 'update':
                    return self.update(id_cliente, nombre,apellidos,telefono)
            else:
                raise web.seeother('/404')
        except Exception as e:
